...
export function parseFeynmanStdoutLine(line: string, ts: string): TranscriptEntry[] {
  ...
  // RPC protocol messages - filter these out
  if (type === "response" || type === "extension_ui_request" || type === "extension_ui_response" || type === "extension_error") {
    return [];
  }

  if (type === "agent_start") {
    return [{ kind: "system", ts, text: "Feynman agent started" }];
  }

  if (type === "agent_end") {
    return [{ kind: "system", ts, text: "Feynman agent finished" }];
  }
