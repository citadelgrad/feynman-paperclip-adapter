...
export function printFeynmanStreamEvent(raw: string, _debug: boolean): void {
  const line = raw.trim();
  if (!line) return;
  ...
  if (type === "agent_start") {
    console.log(pc.blue("Feynman agent started"));
    return;
  }

  if (type === "agent_end") {
    console.log(pc.blue("Feynman agent finished"));
    return;
  }
