...
const FEYNMAN_SESSIONS_DIR = path.join(os.homedir(), ".feynman", "paperclips");
...
/**
 * Inject Paperclip skills into Feynman's agent skills directory
 * (~/.feynman/agent/skills/ instead of ~/.pi/agent/skills/).
 */
async function ensureFeynmanSkillsInjected(onLog: AdapterExecutionContext["onLog"]) {
  const skillsDir = await resolvePaperclipSkillsDir();
  if (!skillsDir) return;

  const feynmanSkillsHome = path.join(os.homedir(), ".feynman", "agent", "skills");
  await fs.mkdir(feynmanSkillsHome, { recursive: true });

  const entries = await fs.readdir(skillsDir, { withFileTypes: true });
  for (const entry of entries) {
    if (!entry.isDirectory()) continue;
    const source = path.join(skillsDir, entry.name);
    const target = path.join(feynmanSkillsHome, entry.name);
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[paperclip] Injected Feynman skill "${entry.name}" into ${feynmanSkillsHome}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[paperclip] Failed to inject Feynman skill "${entry.name}" into ${feynmanSkillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
async function ensureSessionsDir(): Promise<string> {
  await fs.mkdir(FEYNMAN_SESSIONS_DIR, { recursive: true });
  return FEYNMAN_SESSIONS_DIR;
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  await onLog("stderr", `[feynman] Resolving Feynman installation (command: ${feynmanCommand})...\n`);
  const installation = await resolveFeynman(feynmanCommand);
  await onLog("stderr", `[feynman] Found Feynman at ${installation.feynmanRoot}\n`);

  // Ensure sessions directory exists
  await ensureSessionsDir();

  // Inject Paperclip skills into Feynman's skills directory
  await ensureFeynmanSkillsInjected(onLog);

  // Build environment with Feynman-specific variables
  ...
  // Feynman-specific environment variables
  env.PI_CODING_AGENT_DIR = path.join(os.homedir(), ".feynman", "agent");
  env.FEYNMAN_SESSION_DIR = path.join(os.homedir(), ".feynman", "sessions");
  env.FEYNMAN_MEMORY_DIR = path.join(os.homedir(), ".feynman", "memory");
  ...
  await ensureFeynmanModelConfiguredAndAvailable({
    model,
    feynmanCommand,
    cwd,
    env: runtimeEnv,
  ...
  if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stderr",
      `[feynman] Session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${cwd}".\n`,
    );
  }
  ...
  if (resolvedInstructionsFilePath) {
    try {
      ...
      await onLog(
        "stderr",
        `[feynman] Loaded agent instructions file: ${resolvedInstructionsFilePath}\n`,
      );
    } catch (err) {
      ...
      await onLog(
        "stderr",
        `[feynman] Warning: could not read agent instructions file "${resolvedInstructionsFilePath}": ${reason}\n`,
      );
  ...
  const runAttempt = async (sessionFile: string) => {
    ...
    if (onMeta) {
      await onMeta({
        ...
        command: installation.nodebin,
        cwd,
        commandNotes: [
          `Feynman root: ${installation.feynmanRoot}`,
          `Pi CLI: ${installation.piCli}`,
          ...commandNotes,
        ],
        commandArgs: args,
    ...
    // Spawn Pi via Feynman's bundled Node.js
    const proc = await runChildProcess(runId, installation.nodebin, args, {
      cwd,
      env: runtimeEnv,
      timeoutSec,
      graceSec,
  ...
  const toResult = (
    ...
    const fallbackErrorMessage = stderrLine || `Feynman exited with code ${rawExitCode ?? -1}`;
  ...
  if (
    ...
    await onLog(
      "stderr",
      `[feynman] Session "${runtimeSessionId}" is unavailable; retrying with a fresh session.\n`,
    );
